  ...
  <div class="app-container">
    ...
    <el-tabs v-model="activeTab" @tab-change="handleTabChange">
      ...
      <el-tab-pane label="设备列表" name="equipment">
        <el-card shadow="hover">
          <el-table v-loading="equipmentLoading" :data="equipmentList" size="small">
            ...
            <el-table-column label="设备编号" align="center" key="deviceCode" prop="deviceCode" width="160">
              <template #default="scope">
                <span style="font-family:monospace;font-size:12px">{{ scope.row.deviceCode || '-' }}</span>
              </template>
            </el-table-column>
            <el-table-column label="设备名称" align="center" key="deviceName" prop="deviceName" :show-overflow-tooltip="true" />
            <el-table-column label="设备类型" align="center" key="deviceType" prop="deviceType" width="100">
              <template #default="scope">{{ deviceTypeLabel(scope.row.deviceType) }}</template>
            </el-table-column>
            <el-table-column label="状态" align="center" key="status" prop="status" width="80">
              <template #default="scope">
                <el-tag :type="statusType[scope.row.status] || 'info'" size="small">{{ statusLabel(scope.row.status) }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="固件版本" align="center" key="firmwareVersion" prop="firmwareVersion" width="100" />
            <el-table-column label="最后在线" align="center" key="lastSeenAt" prop="lastSeenAt" width="160">
              <template #default="scope">{{ parseTime(scope.row.lastSeenAt) || '-' }}</template>
            </el-table-column>
            <el-table-column label="操作" align="center" width="100">
              <template #default="scope">
                <el-button size="small" link type="primary" icon="View" @click="handleViewDevice(scope.row)">详情</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
...
import { getManufacturer, getManufacturerDetail, resetManufacturerApiKey, updateManufacturerSettings, getManufacturerEquipment } from "@/api/iot/manufacturer"
...
const statusType = { online: 'success', offline: 'info', error: 'danger', maintenance: 'warning', '1': 'success', '0': 'info' }
const statusLabel = (s) => ({ online: '在线', offline: '离线', error: '故障', maintenance: '维护', '1': '在线', '0': '离线' }[s] || s || '-')
const deviceTypeLabel = (t) => ({ treadmill: '跑步机', elliptical: '椭圆机', rowing_machine: '划船机', resistance_trainer: '阻力训练', spin_bike: '动感单车' }[t] || t || '-')
...
function loadEquipment() {
  ...
  getManufacturerEquipment(manufacturerId).then(res => {
    equipmentList.value = res.rows || res || []
  }).catch(() => {
    equipmentList.value = []
  }).finally(() => {
...
function handleViewDevice(row) {
  router.push('/iot/device?id=' + row.deviceId)
}
