...
import { getDashboardKpi, getDashboardTrends, getDashboardAlerts } from "@/api/iot/dashboard"
...
function loadKPI() {
  getDashboardKpi().then(res => {
    ...
    // Fallback: device-count based estimation
...
function loadTopDevices() {
  listDevice({ pageNum: 1, pageSize: 10, manufacturerId: filterParams.manufacturerId }).then(res => {
    topDevices.value = (res.rows || []).map(d => ({
      ...d,
      todayWorkouts: Math.round(10 + Math.random() * 30),
      todayReps: Math.round(500 + Math.random() * 2000),
      onlineRate: Math.random() * 0.3 + 0.7
    }))
  })
}
